  ...
  <div v-if="lead !== null" v-show="hidden">
    ...
    <div class="tablet">
      <div class="tablet__head tablet__head__color-brand">
        <h3 class="tablet__head-title text-white">
          {{ trans("Information") }}
        </h3>
          ...
          >
            <div class="row">
              ...
              <div class="col-xs-3 no-padding">
                <button
                  class="btn btn-brand btn-full-width cta-btn"
              ...
              >
                <button
                  ...
                  @click="
                    openConfirmModal(
                      reopenLead,
                      'Are you sure you want to open the lead?'
                    )
                  "
                >
              ...
              <div class="col-xs-9">
                <p>
                  {{ lead.visible_deadline_date }}
                  {{ lead.visible_deadline_time }}
                </p>
              </div>
              <div class="col-xs-3">
                {{ trans("Status") }}
              ...
              <div class="col-xs-3">
                {{ trans("Purchase Method") }}
              </div>
              <div class="col-xs-9">
                <p>{{ lead.purchase_method }}</p>
              </div>
            </div>
            <br>
            <div class="col-xs-3 no-padding">
        ...
        <div class="row">
          ...
          <div class="col-xs-6">
            <p
              aria-hidden="true"
              data-toggle="tooltip"
              :title="trans('Contact person name')"
            >
              {{ lead.client.primary_contact.name }}
            </p>
          </div>
          <div class="col-xs-6">
            <p>
              <a :href="'mailto:' + lead.client.primary_contact.email">{{
                lead.client.primary_contact.email
              }}</a>
            </p>
          </div>
          <div class="col-xs-6">
            <p
              aria-hidden="true"
              data-toggle="tooltip"
              :title="trans('Primary number')"
            >
              {{ lead.client.primary_contact.primary_number }}
            </p>
          </div>
          <div class="col-xs-6">
            <p
              aria-hidden="true"
              data-toggle="tooltip"
              :title="trans('Secondary number')"
            >
              {{ lead.client.primary_contact.secondary_number }}
            </p>
          </div>
          <div class="col-xs-6">
            <p
              aria-hidden="true"
              data-toggle="tooltip"
              :title="trans('Company type')"
            >
              {{ lead.client.company_type }}
            </p>
          </div>
          <div class="col-xs-6">
            <p aria-hidden="true" data-toggle="tooltip" :title="trans('Vat')">
              {{ lead.client.vat }}
            </p>
          </div>
          <div class="col-xs-12">
            <p
              ...
              :title="trans('Address')"
            >
              {{ lead.client.address }}, {{ lead.client.zipcode }},
              {{ lead.client.city }}
            </p>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    closeLead() {
      axios.post("/leads/updatestatus/" + this.lead.external_id, {
        closeLead: true,
      });
    ...
    deleteLead() {
      axios.delete("/leads/" + this.lead.external_id + "/json");
