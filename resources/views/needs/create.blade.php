...
@section('heading')
    {{ __('Create need') }} <span class="small">{{ $client ? '(' . $client->company_name . ')' : '' }}</span>
@stop

@section('content')

    <div class="row">
        <form action="{{ route('needs.store') }}" method="POST" id="createneedForm" enctype="multipart/form-data">
            {{ csrf_field() }}
            <div class="col-sm-8">
                <div class="tablet">
                ...
                @if (Entrust::can('need-upload-files') && $filesystem_integration)
                    <div class="tablet">
                        <div class="tablet__body">
            ...
            <div class="col-sm-4">
                <div class="tablet">
                    <div class="tablet__body">
                        ...
                        <div class="form-group">
                            <input type="submit" class="btn btn-md btn-brand movedown" id="createneed"
                                value="{{ __('Create need') }}">
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('#client_external_id').on('changed.bs.select', function(e, clickedIndex, isSelected, previousValue) {
                ...
                if (value == "new_client") {
                    window.location.href = "/needs/create"
                }
            });
            ...
            myDropzone = null;
            @if (Entrust::can('need-upload-files') && $filesystem_integration)
                var myDropzone = new Dropzone("#createneedForm", {
                ...
                window.location.href = ("/needs/"+response.need_external_id)
                });
            ...
            $('input[type="submit"]').on("click", function(e) {
                ...
                if (myDropzone != null && myDropzone.getQueuedFiles().length > 0) {
                    ...
                    $.ajax({
                        type: 'post',
                        url: '{{ route('needs.store') }}',
                        data: $("#createneedForm").serialize(),
