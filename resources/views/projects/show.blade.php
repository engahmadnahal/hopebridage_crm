                ...
                <nav class="navbar board text-black ">
                    @if (!$project->isClosed())
                        <a href="{{ route('client.project.task.create', [$client->external_id, $project->external_id]) }}"
                            class="btn btn-md btn-brand" style="margin:1em;">@lang('New task')</a>
                    @endif
                </nav>
                <div class="project-board-lists">
                    @foreach ($statuses as $status)
                        <div class="project-board-list">
                            ...
                            <ul class="sortable" id="{{ $status->title }}"
                                data-status-external-id="{{ $status->external_id }}" style="min-height: 32em;">
                                        ...
                                        @if ($task->status_id == $status->id)
                                            <div class="project-board-card-wrapper">
                                                <div class="project-board-card">
                                                    ...
                                                    <p class="project-board-card-title"><a
                                                            href="{{ route('tasks.show', $task->external_id) }}"
                                                            class="link-color">{{ $task->title }}</a></p>
                                                    <div class="project-board-card-description">{!! str_limit($task->description, 154, '...') !!}
                                                    </div>
                                                </div>
                                                <div class="project-board-card-footer">
                                                    <ul class="list-inline" style="padding: 8px; min-height: 3.3em;">
                                                        <li class="project-board-card-list">{{ $task->created_at }}</li>
                                                        <li class="project-board-card-list">{{ $task->deadline }}</li>
                                                        <li class="project-board-card-thumbnail text-right"
                                                            style="float:right;">
                                                            <a href="{{ route('users.show', $task->user->external_id) }}"><img
                                                                    src="{{ $task->user->avatar }}"
                                                                    class="project-board-card-thumbnail-image"
                                                                    title="{{ $task->user->name }}" /></a>
                                                        </li>
                                                    </ul>
                                                </div>
                                            </div>
                        ...
                        @foreach ($collaborators as $collaborator)
                            <li>
                                <a href="{{ route('users.show', $collaborator->external_id) }}">
                                    <img src="{{ $collaborator->avatar }}" class="project-board-card-thumbnail-image"
                                                                ...
                                                                <span class="nav-link">
                                                                    ...
                                                                    <form method="POST"
                                                                        ...
                                                                        <input type="hidden" name="_token"
                                                                            value="{{ csrf_token() }}" />
                                                                        <button type="submit"
                                                                            class="btn btn-clean nav-link-text">{{ __('Delete') }}</button>
                                                                    </form>
                                                                </span>
                                                            </li>
                                                        @endif
