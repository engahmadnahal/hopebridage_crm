                        ...
                        <select name="user_assigned_id"
                            ...
                            @foreach ($users as $key => $user)
                                <option data-tokens="{{ $user }}"
                                    {{ $lead->user_assigned_id == $key ? 'selected' : '' }}
                                    value="{{ $key }}">
                                    {{ $user }}
                                </option>
                            @endforeach
...
<div class="row margin-top-10">
    ...
    <div class="col-md-9">
        <span {{ Entrust::can('lead-update-deadline') ? 'data-toggle=modal data-target=#ModalFollowUp' : '' }}
            class="siderbar-list-value {{ $lead->isCloseToDeadline() ? 'text-danger' : '' }}">{{ date(carbonFullDateWithText(), strTotime($lead->deadline)) }}
...
<div class="row margin-top-10">
                    ...
                    <form method="POST" action="{{ url('leads/updatestatus', $lead->external_id) }}">
                        ...
                        <select name="status_id"
                            ...
                            @foreach ($statuses as $key => $status)
                                <option {{ $lead->status->id == $key ? 'selected' : '' }}
                                    value="{{ $key }}">
                                    {{ $status }}</option>
                            @endforeach
                        </select>
    ...
    <br>
    <br>
    <div class="col-md-3">{{ __('Purchase Method') }}</div>
    <div class="col-md-9">
        <span id="#" class="siderbar-list-value">
...
<div class="row margin-top-10">
    <div class="col-md-3">{{ __('Expiration Date') }}</div>
    <div class="col-md-9">
        <span id="#" class="siderbar-list-value">
            {{ $lead->payment_date }}
        </span>
...
<div class="row margin-top-10">
    <div class="col-md-3">{{ __('Budget') }}</div>
    <div class="col-md-9">
        <span id="#" class="siderbar-list-value">
            {{ $lead->budget }}
