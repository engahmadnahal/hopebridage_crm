...
class UsersController extends Controller
{
    ...
    public function anyData()
    {
        ...
        return Datatables::of($users)
            ->addColumn(
                'namelink',
                '<a href="{{ route("users.show",[$external_id]) }}">{{$name}}</a>'
            )
            ...
            ->addColumn('edit', function ($user) {
                return '<a href="' .
                    route('users.edit', $user->external_id) .
                    '" class="btn btn-link">' .
                    __('Edit') .
                    '</a>';
            })
    ...
    public function leadData($id)
    {
        ...
        return Datatables::of($leads)
            ->addColumn(
                'titlelink',
                '<a href="{{ route("leads.show",[$external_id]) }}">{{$title}}</a>'
            )
            ->editColumn('created_at', function ($leads) {
                return $leads->created_at
                    ? with(new Carbon($leads->created_at))->format(carbonDate())
                    : '';
            })
            ->editColumn('deadline', function ($leads) {
                return $leads->deadline
                    ? with(new Carbon($leads->deadline))->format(carbonDate())
                    : '';
            })
    ...
    public function clientData($id)
    {
        ...
        return Datatables::of($clients)
            ->addColumn('clientlink', function ($clients) {
                return '<a href="' .
                    route('donors.show', $clients->external_id) .
                    '">' .
            ...
            ->editColumn('created_at', function ($clients) {
                return $clients->created_at
                    ? with(new Carbon($clients->created_at))->format(
                        carbonDate()
                    )
                    : '';
            })
    ...
    public function store(StoreUserRequest $request)
    {
        ...
        if (User::count() >= $settings->max_users) {
            Session::flash(
                'flash_message_warning',
                __('Max number of users reached')
            );
            return redirect()->back();
        }
        $path = null;
    ...
    public function show($external_id)
    {
        ...
        return view('users.show')
            ...
            ->with(
                'task_statistics',
                $user->totalOpenAndClosedTasks($external_id)
            )
            ->with(
                'lead_statistics',
                $user->totalOpenAndClosedLeads($external_id)
            )
    ...
    public function update($external_id, UpdateUserRequest $request)
    {
        ...
        if ($role && $role->name == Role::OWNER_ROLE && $owners->count() <= 1) {
            Session()->flash(
                'flash_message_warning',
                __(
                    'Not able to change owner role, please choose a new owner first'
                )
            );
        } else {
            if (
    ...
    public function destroy(Request $request, $external_id)
    {
        ...
        if ($user->hasRole('owner')) {
            return Session()->flash(
                'flash_message_warning',
                __('Not allowed to delete super admin')
            );
        }
        ...
        if (
            $request->clients == 'move_all_clients' &&
            $request->client_user != ''
        ) {
        ...
        try {
            ...
            Session()->flash(
                'flash_message_warning',
                __(
                    'User can NOT have, leads, clients, or tasks assigned when deleted'
                )
            );
        }
    ...
    private function allRoles()
    {
        if (
            auth()
                ->user()
                ->roles->first()->name == Role::OWNER_ROLE
        ) {
            return Role::all(
                'display_name',
                'id',
        ...
        return Role::all('display_name', 'id', 'name', 'external_id')
