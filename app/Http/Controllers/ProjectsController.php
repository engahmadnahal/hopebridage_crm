...
//                'deadline',
        ...
        return DataTables::of($projects)
            ->addColumn(
                'titlelink',
                '<a href="{{ route("projects.show",[$external_id]) }}">{{$title}}</a>'
            )
...
//            })
            ->editColumn('deadline', function ($projects) {
                return $projects->created_at
                    ? with(new Carbon($projects->startDate))->format(
                        carbonDate()
                    )
                    : '';
            })
            ->editColumn('created_at', function ($projects) {
                return $projects->created_at
                    ? with(new Carbon($projects->created_at))->format(
                        carbonDate()
                    )
                    : '';
            })
            ...
            ->addColumn('view', function ($projects) {
                return '<a href="' .
                    ...
                    route('projects.destroy', $projects->external_id) .
                    '" data-title="' .
                    $projects->title .
    ...
    {
        return view('projects.index')->withStatuses(Status::typeOfTask()->get());
    }
    ...
    public function store(StoreProjectRequest $request)
    {
        ...
        $project = Project::create([
            ...
            'client_external_id' => $client ? $client->id : null,
            'client_id' => $client ? $client->id : null,
            'user_assigned_id' => $request->user_assigned_id ? $request->user_assigned_id : 1,
...
//        ]);
    ...
    public function updateDeadline(Request $request, $external_id)
    {
        ...
        $input = $request = [
            'deadline' =>
                $request->deadline_date . ' ' . $request->deadline_time . ':00',
        ];
        $project->fill($input)->save();
