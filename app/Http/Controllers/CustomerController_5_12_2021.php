...
class CustomerController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // return redirect("Customer/" . $obj->id . "/edit");
...
//            (($request->get('citizin')) == 1 ? $mowaten : $ref)+
    ...
    {
//        return DataTables::of(Post::with('jehato','posttype')
        ...
        return DataTables::of(Customer::with('getstate', 'getregion', 'gettype', 'user')->withCount('projects')->filter()
//               ->where(function ($q) use ($name,$type,$file_no,$card_no,$total,$total_to,$state,$region,$material,$housetype,$child_no,$child_no_to){
//                   if(isset($name))
...
//               })
            ...
            ->addColumn('action', function ($query) {
                ...
                if (in_array(30, parent::$data["allowedActions"])) {
                    if (!$query->rejection)
                        $link = $link . '<a href="' . url('' . parent::$data["route"] . '/reject/' . $query->id . '') . '" class="btn btn-circle btn-icon-only btn-info reject_btn" title="رفض"><i class="fa fa-eye-slash"></i> </a>';
                    else
                        $link = $link . '<a href="' . url('' . parent::$data["route"] . '/recovery/' . $query->id . '') . '" class="btn btn-circle btn-icon-only btn-success recovery_btn"  title="استرجاع"><i class="fa fa-eye"></i> </a>';
...
<label class="checkbox-inline parent-check">
    ...
    public function CustomerFamily($id)
    {
        ...
        return DataTables::of(Family::with('jender', 'health', 'rel')
            ...
            ->addColumn('action', function ($query) {
//                if((in_array(31,parent::$data["allowedActions"])) || (in_array(51,parent::$data["allowedActions"])))
//                return '<a pull-link="Family/'. $query->id .'/update" id="update_family" class="btn btn-circle btn-icon-only btn-green" title="تعديل" id="edit_btn"> <i class="fa fa-edit"></i> </a>'
