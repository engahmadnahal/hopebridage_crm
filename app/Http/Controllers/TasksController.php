...
class TasksController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->middleware('task.assigned', [
            'only' => ['updateAssign', 'updateTime'],
        ]);
    }
    ...
    {
        return view('tasks.index')->withStatuses(Status::typeOfTask()->get());
    }
    ...
    public function anyData()
    {
        ...
        return DataTables::of($tasks)
            ->addColumn(
                'titlelink',
                '<a href="{{ route("tasks.show",[$external_id]) }}">{{$title}}</a>'
            )
            ...
            ->editColumn('created_at', function ($tasks) {
                return $tasks->created_at
                    ? with(new Carbon($tasks->created_at))->format(carbonDate())
                    : '';
            })
            ->editColumn('deadline', function ($tasks) {
                return $tasks->created_at
                    ? with(new Carbon($tasks->deadline))->format(carbonDate())
                    : '';
            })
            ...
            ->addColumn('view', function ($tasks) {
                return '<a href="' .
                    route('tasks.show', $tasks->external_id) .
                    '" class="btn btn-link">' .
                    __('View') .
                    '</a>' .
                    '<a data-toggle="modal" data-id="' .
                    route('tasks.destroy', $tasks->external_id) .
                    '" data-target="#deletion" class="btn btn-link">' .
                    __('Delete') .
                    '</a>';
            })
        ...
        return view('tasks.create')
            ->withUsers(
                User::with(['department'])
                    ->get()
                    ->pluck('nameAndDepartmentEagerLoading', 'id')
            )
            ...
            ->with(
                ...
                Integration::whereApiType('file')->first()
            );
    }
    ...
    public function store(StoreTaskRequest $request)
    {
        ...
        if ($request->project_external_id) {
            $project = Project::whereExternalId(
    ...
    private function upload($image, $task)
    {
        ...
        if (
            !auth()
                ->user()
                ->can('task-upload-files')
        ) {
            session()->flash(
                'flash_message_warning',
                __('You do not have permission to upload images')
            );
        ...
        if ($totaltsize > 15) {
            Session::flash(
                'flash_message',
                __('File Size cannot be bigger than 15MB')
            );
            return redirect()->back();
        }
    ...
    public function show(Request $request, $external_id)
    {
        ...
        return view('tasks.show')
            ...
            ->withStatuses(Status::typeOfTask()->pluck('title', 'id'))
            ->withProjects(
                $task->client->projects()->pluck('title', 'external_id')
            )
    ...
    public function updateStatus($external_id, Request $request)
    {
        if (
            !auth()
                ->user()
                ->can('task-update-status')
        ) {
            session()->flash(
                'flash_message_warning',
                __('You do not have permission to change task status')
            );
        ...
        if ($request->ajax() && isset($input['statusExternalId'])) {
            $input['status_id'] = Status::whereExternalId(
                $input['statusExternalId']
            )->first()->id;
        }
    ...
    public function updateProject($external_id, Request $request)
    {
        ...
        if ($request->project_external_id) {
            $project_id = Project::whereExternalId(
                $request->project_external_id
            )->first()->id;
        }
    ...
    public function updateDeadline(Request $request, $external_id)
    {
        if (
            !auth()
                ->user()
                ->can('task-update-deadline')
        ) {
            session()->flash(
                'flash_message_warning',
                __('You do not have permission to change task deadline')
            );
            return redirect()->route('tasks.show', $external_id);
        }
        $task = $this->findByExternalId($external_id);
        $task
            ->fill(['deadline' => Carbon::parse($request->deadline_date)])
            ->save();
